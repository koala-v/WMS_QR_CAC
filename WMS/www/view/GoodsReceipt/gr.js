    ...
    function (
        ...
        $scope.openCam = function (type) {
            if (!ENV.fromWeb) {
                ...
                } else if (is.equal(type, 'QrCode')) {
                    $cordovaBarcodeScanner.scan().then(function (imageData) {
                        $scope.Detail.Scan.QrCode = imageData.text;
                        $scope.showQrCode($scope.Detail.Scan.QrCode);
                    }, function (error) {
                        $cordovaToast.showShortBottom(error);
                    });
                }
        ...
        $scope.showQrCode = function (QrCode) {
            if (QrCode.length > 0) {
                var arrQrCode = QrCode.split('/');

                $scope.Detail.QrCode.Qr1 = arrQrCode[0] + "  " + arrQrCode[1] + "  " + arrQrCode[2] + "  " + arrQrCode[3] + "  " + arrQrCode[4];
                $scope.Detail.QrCode.Qr2 = +"  " + arrQrCode[5] + "  " + arrQrCode[6] + "  " + arrQrCode[7] + "  " + arrQrCode[8];
            }
        };
